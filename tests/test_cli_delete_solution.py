...
class TestCliDeleteSolution(object):
    ...
    def test_cli_delete_solution_001(self, snippy, mocker):
        ...
        cause = snippy.run(['snippy', 'delete', '--solution', '-d', '61a24a156f5e9d2d'])  ## workflow
    ...
    def test_cli_delete_solution_002(self, snippy, mocker):
        ...
        ##        category.
        content_read = {Solution.BEATS_DIGEST: Solution.DEFAULTS[Solution.BEATS]}
        cause = snippy.run(['snippy', 'delete', '-d', '61a24a156f5e9d2d'])  ## workflow
        assert cause == Cause.ALL_OK
        assert len(Database.get_solutions()) == 1
        Content.verified(mocker, snippy, content_read)
    ...
    def test_cli_delete_solution_003(self, snippy, mocker):
        ...
        content_read = {Solution.BEATS_DIGEST: Solution.DEFAULTS[Solution.BEATS]}
        cause = snippy.run(['snippy', 'delete', '--solution', '-d', '61a24'])  ## workflow
        assert cause == Cause.ALL_OK
        assert len(Database.get_solutions()) == 1
        Content.verified(mocker, snippy, content_read)
    ...
    def test_cli_delete_solution_004(self, snippy, mocker):
        ...
        content_read = {Solution.BEATS_DIGEST: Solution.DEFAULTS[Solution.BEATS]}
        cause = snippy.run(['snippy', 'delete', '--solution', '-d', '61a24a156f5e9d2d448915eb68ce44b383c8c00e8deadbf27050c6f18cd86afe'])  ## workflow # pylint: disable=line-too-long
        assert cause == Cause.ALL_OK
        assert len(Database.get_solutions()) == 1
        Content.verified(mocker, snippy, content_read)

    @pytest.mark.usefixtures('nginx')
    def test_cli_delete_solution_005(self, snippy):
        ...
        cause = snippy.run(['snippy', 'delete', '--solution', '-d', ''])  ## workflow
        assert cause == Cause.ALL_OK
    ...
    def test_cli_delete_solution_006(self, snippy, mocker):
        ...
        cause = snippy.run(['snippy', 'delete', '--solution', '-d', '123456789abcdef0'])  ## workflow
    ...
    def test_cli_delete_solution_008(self, snippy, mocker):
        ...
        cause = snippy.run(['snippy', 'delete', '--solution', '-d', '123456'])  ## workflow
    ...
    def test_cli_delete_solution_009(self, snippy, mocker):
        ...
        cause = snippy.run(['snippy', 'delete', '--solution', '--content', data])  ## workflow
        assert cause == Cause.ALL_OK
        assert len(Database.get_solutions()) == 1
        Content.verified(mocker, snippy, content_read)
    ...
    def test_cli_delete_solution_010(self, snippy, mocker):
        ...
        cause = snippy.run(['snippy', 'delete', '--solution', '--content', 'not-exists'])  ## workflow
    ...
    def test_cli_delete_solution_012(self, snippy, mocker):
        ...
        cause = snippy.run(['snippy', 'delete', '--solution', '--content', ''])  ## workflow
