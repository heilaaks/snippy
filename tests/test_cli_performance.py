...
class TestCliPerformance(object):
    ...
    def test_cli_performance(self, snippy_perf, capsys, caplog):
        ...
        Reference PC:   1 loop :  0.0238 /   55 loop :  0.9991 / 100 loop : 1.7988
        Reference PC: 880 loop : 15.6723 / 1000 loop : 17.9755
        ...
        for _ in range(55):
            ...
            assert cause == Cause.ALL_OK
            cause = snippy_perf.run(['snippy', 'delete', '-d', 'a5dd8f3807e08420'])
            assert cause == Cause.ALL_OK
            cause = snippy_perf.run(['snippy', 'delete', '-d', '27ddfbc3d289f29a'])
            assert cause == Cause.ALL_OK
